...
3. **The ambient wave is scroll-independent (supersedes the `settle` beat).** Owner request: the waviness no longer straightens as you scroll — it is a constant ambient presence driven only by the scene clock (full weight everywhere, still frozen under reduced motion). `settle` is removed from `FlowTimeline`/`flowTimeline` and its tests rather than left dead; the page now opens and ends wavy, with the street and planet exit as the only scroll-driven changes.
4. **The ambient wave is removed outright (supersedes Change 3 after in-browser review).** The waviness itself went: outside the wake, streamlines are now straight ruled dashes — laminar chart lines that bend only around the bodies and weave only in the street. `wobble`/`wobbleWavelength`/`wobbleOmega` leave `FlowField`; with no time-dependent term left, `fieldVelocity` and `integrateStreamline` drop their `time` parameter entirely and the field is steady. `WOBBLE_AMPLITUDE`, the two wobble constants in the component, and the wobble test are deleted.
