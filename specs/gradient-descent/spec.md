...
## Changes (2026-08-31 — owner revision: the field states its direction)

1. **Quiver arrowheads on the field (amends the rendering criterion "dashed field lines flowing along −∇f").** Owner request: the vector direction — and with it the 3D space — must be readable at a glance, not inferred from dash travel. Every field streamline now ends in a solid chevron arrowhead at its downstream tip: the field becomes a quiver figure, every arrow pointing down −∇f toward the basin. Arrowheads are sized in **world units** (0.18, measured tip-to-barbs) and drawn at the streamline tip's local projection scale, so they shrink as the field recedes — an explicit depth cue. They are solid strokes in the same field ink (no fills, no dash) — `DescentScene.test.tsx` (source contract) asserts the arrowhead drawing and its world-unit sizing.
2. **Sizing uses the foreshortening-free scale (found in screenshot review).** The first pass measured the tip segment's on-screen length to derive px-per-world-unit — which collapses to near-zero for streamlines pointing away from the camera, rendering those arrows invisible. Shipped sizing is `projectionScale(depth) = focal/depth` (now exported from `descentField.ts` and unit-tested), clamped to 3–24 px so far arrows fade but never vanish. Verified in the browser: ~20 chevrons read coherently at the top of the page and converge radially onto the crosshair in the basin.
3. **The field chevrons are smaller (owner revision, after the filled path head landed).** Beside the smaller filled head on the traversed tip, the open chevrons read heavy. `ARROW_SIZE` 0.18 → **0.12** world units and the clamp tightens to 3–16 px. The field stays directional but quieter; the accent path and crosshair keep the figure's hierarchy.

## Changes (2026-08-31 — owner revision: the run states its heading)

1. **The traversed path carries an arrowhead at its leading tip (owner review of the mid-descent frame).** Mid-page the red path ends far from the crosshair — geometrically correct, since only the traversed portion is red and the dotted remainder leads on — but without a terminal mark the path read as stopping mid-air rather than heading to the minimum. The tip now carries an accent chevron pointing along the local tangent: the traveler's heading, visibly aiming down the dotted remainder toward the crosshair. Same chevron grammar and perspective sizing as the field arrows, stroked at the path's 1.5 px weight; it appears as soon as any distance is covered (two points minimum) and dissolves into the convergence cluster at the end. The field arrows' contract is unchanged — they point along −∇f, which funnels to the global minimum and converges radially onto the crosshair in the basin.
2. **The heading head is a filled triangle, smaller (owner revision of revision 1).** The open chevron at the traversed tip read heavier than intended beside the hairline path. The tip now carries a solid filled triangle at `PATH_ARROW_SIZE = 0.11` world units (versus the field chevron's open 0.18), clamped to 3–18 px, in the same accent. The field chevrons stay open — the quiver grammar is strokes; the traveler's head is the figure's one solid glyph, the way a plotted optimizer path terminates in a marker.
