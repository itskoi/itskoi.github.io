...
3. **Wireframe opacity tuned for paper (resolves open question 1).** Shipped values: lattice `CUBE_BORDER_OPACITY` 0.9 → 0.35, `CUBE_FILL_OPACITY` 0.05 → 0.04. On flat paper the drifting lattice crosses the Education awards column at full ink and reads as noise over the rows (verified by screenshot at 0.9 and 0.5); a third-ink edge keeps it visible as intentional line-art without fighting the type. Chess shards sit slightly under full ink (`EDGE_OPACITY` 0.9 → 0.75, owner-tuned post-review — they remain the specimen but stop competing with the display type). The `FIG. 1` caption also moved to the hero's bottom-right corner — the exploded shard cloud it labels sits center-frame and was crossing the caption in the upper-right.
   - Known follow-up (out of scope, owned by the chess specs): on ≤ ~480px viewports the horizontal field of view excludes the pieces at ±11 units, so no wireframe is visible on mobile. Fixing it requires camera/geometry recomposition.
4. **Certification links styled.** Pre-existing gap surfaced by the flattening: certification anchors carried the browser-default blue. They now follow the page-wide link grammar (ink, red on hover).
